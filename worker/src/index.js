...
- CHARACTER EDGES: ${isMatch
    ? "render the character's edges EXACTLY the way the reference image does. Do NOT add a cartoon die-cut black outline unless the reference itself uses outlines — a photo reference keeps natural photo edges, a watercolor keeps soft watercolor edges. Keep the silhouette crisp against the chroma plate, but do not restyle it."
    : "trace the entire character silhouette with a clean, uniform 2-3px PURE BLACK outline (the boundary between character and the chroma-key background). Apply consistently and identically across ALL 9 cells. This gives the sticker pack a unified \"die-cut sticker\" look. Even photo-realistic stickers should have this clean black outline added."}
